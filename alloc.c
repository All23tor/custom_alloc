...
static block_ptr find_free_block(block_ptr* last_block, size_t size) {
...
static void split_block(block_ptr block, size_t size) {
...
static block_ptr grow_heap(block_ptr last_block, size_t size) {
...
static block_ptr coalesce_blocks(block_ptr block) {
...
static block_ptr get_block_from_ptr(void* ptr) {
...
static void copy_block_data(block_ptr src, block_ptr dest) {
  size_t* src_data = (size_t*)src->memory;
  size_t* dest_data = (size_t*)dest->memory;
  size_t i;
  for (i = 0; i * sizeof(size_t) < src->block_size &&
       i * sizeof(size_t) < dest->block_size;
       i++) {
...
void* custom_realloc(void* ptr, size_t size) {
  ...
  if (is_valid_address(ptr)) {
    ...
    if (block->block_size >= aligned_size) {
      ...
      if (block->next_block && block->next_block->is_free &&
          (block->block_size + block->next_block->block_size + METADATA_SIZE) >=
            aligned_size) {
        coalesce_blocks(block);
        if (block->block_size - aligned_size >= (METADATA_SIZE + 4)) {
          split_block(block, aligned_size);
