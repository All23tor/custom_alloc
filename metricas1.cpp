...
void prueba_visual_linear() {
    ...
    LinearAllocator<int>::init(1024); // Iniciamos una arena pequeña
    LinearAllocator<int> alloc;

    int* p1 = alloc.allocate(1); *p1 = 10;
    int* p2 = alloc.allocate(1); *p2 = 20;
...
void medir_metricas_estandar(const std::string& nombre, int num_elementos) {
    ...
    { // SCOPE: Al salir de aquí, se llama a deallocate/destructores
        ...
        tiempo_alloc = std::chrono::duration<double, std::milli>(end - start).count();
        ...
        tiempo_free = std::chrono::duration<double, std::milli>(end - start).count();
    }

    std::cout << nombre;
    if (nombre.length() < 8) std::cout << "\t";
...
void prueba_velocidad_pura(const std::string& nombre, int n) {
    ...
    double t_alloc = std::chrono::duration<double, std::milli>(end - start).count();
    ...
    double t_free = std::chrono::duration<double, std::milli>(end - start).count();
...
void prueba_estres_memoria(const std::string& nombre) {
    ...
    for (int i = 1; i <= 5; ++i) {
        ...
        std::cout << "  Ciclo " << i << " | RAM Acumulada: " << diferencia << " KB\n";
    }
    std::cout << "--------------------------------\n";
}
...
int main() {
    ...
    LinearAllocator<int>::init(1024 * 1024 * 100); 
